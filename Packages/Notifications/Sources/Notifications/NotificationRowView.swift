...
struct NotificationRowView: View {
  ...
  nonisolated private func makeAppendView(type: Models.Notification.NotificationType) -> Text {
    if notification.accounts.count > 1 {
      ...
      if type == .favourite, let favCount = notification.status?.favouritesCount, favCount > 1 {
        othersCount = favCount - 1
      } else if type == .reblog, let reblogCount = notification.status?.reblogsCount, reblogCount > 1 {
